	...
	public DaoS() throws ClassNotFoundException, SQLException{
		...
		*/
		...
		public int updateProduct(String productName, int productPrice, int productCode) throws SQLException {
			PreparedStatement pst = connection.prepareStatement("UPDATE m_product SET product_name = ?, price = ?, update_datetime = NOW() WHERE product_code = ? "
					+ "AND product_code NOT IN(SELECT product_code FROM t_sales) AND delete_datetime IS NULL");
		...
		public boolean addSales(String date, int productCode, int quantity) throws SQLException {
			...
			if(check.next()) {
				...
				PreparedStatement pst = connection.prepareStatement("INSERT INTO t_sales(sales_date, product_code, quantity, register_datetime, update_datetime) "
						+ "values(?, ?, ?, NOW(), NOW())");
		...
		public void export(List<TotalSellByProduct> totalSellByProduct, OutputStream outputStream) throws IOException {
	        ...
	        for (TotalSellByProduct sale : totalSellByProduct) {
	            ...
	            String formattedProductCode = String.format("%03d", sale.getProductCode());
	        	outputResult = outputResult + formattedProductCode + "," + productName + "," + sale.getUnitPrice() + "," + sale.getQuantity() + "," + sale.getAmount() + "\n";
	        }
