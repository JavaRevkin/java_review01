...
public class DaoS {
	...
	public DaoS() throws ClassNotFoundException, SQLException{
		...
		*/
		public List<Product> getAllProducts() throws SQLException {
			...
			List<Product> products = new ArrayList<Product>();
				...
				while(rs.next()) {
					...
					//product.setProductCode(Integer.parseInt(rs.getString("product_name")));
		...
		public int deleteProduct(int productCode) throws SQLException {
			PreparedStatement pst = connection.prepareStatement("UPDATE m_product SET delete_datetime = NOW() WHERE product_code = ? AND delete_datetime IS NULL AND product_code NOT IN (select product_code from t_sales)");
		...
		public void export(List<TotalSellByProduct> totalSellByProduct, OutputStream outputStream) throws IOException {
		    ...
		    String outputResult = "\"商品コード\",\"商品名\",\"単価\",\"数量\",\"金額\"\n";
		    ...
		    for (TotalSellByProduct sale : totalSellByProduct) {
		        ...
		        String unitPriceFormatted = numberFormat.format(sale.getUnitPrice());
		        String quantityFormatted = numberFormat.format(sale.getQuantity());
		        String amountFormatted = numberFormat.format(sale.getAmount());

		        String productCodeFormatted = "\"" + sale.getProductCode() + "\"";
		        String productNameFormatted = "\"" + productName + "\"";
		        String unitPriceAndAmountFormatted = "\"" + unitPriceFormatted + "\",\"" + amountFormatted + "\"";
		        String QuantityFormatted = "\"" + quantityFormatted + "\"";

		        outputResult = outputResult + productCodeFormatted + "," + productNameFormatted + "," + QuantityFormatted + "," + unitPriceAndAmountFormatted + "\n";
		    }
