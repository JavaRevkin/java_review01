    ...
    public static void main(String[] args) {
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(inputFilePath), "SJIS"));
            ...
            while ((line = br.readLine()) != null) {
                ...
                try {
                    intUnitPrice = Integer.parseInt(Price);
                ...
                while (true) {
                    System.out.print("商品コード：" + productCode + "\n商品名：" + productName + "\n単価：" + numberFormat.format(intUnitPrice) + "\n販売数：\n[画面入力] ");
                    intSalesQuantity = scanner.nextInt();
                    if (intSalesQuantity >= 1 && intSalesQuantity <= 100) {
                        break;
                    } else {
                ...
                int intSalesAmount = intUnitPrice * intSalesQuantity;
                String formattedSalesAmount = numberFormat.format(intSalesAmount);
                int intPrice = Integer.parseInt(Price);

                bw.write(productCode + "," + productName + "," + numberFormat.format(intPrice) + "," + intSalesQuantity + "," + formattedSalesAmount);
                bw.newLine();
            }
