...
/**
 * @file 2-6.c
 * @brief	 2-5からソート（並べ替え）を実行できるプログラム
...
void sortMenu(member **pre_member);
void doSort(member **pre_member, int changeWord, int ascOrDesc);
void swap(member* person, member**pre_member);
int Count(member *person);
...
void main() {
	...
	while(1){
		...
		switch(choice[0]){
			...
			case '6': sortMenu(&pre_member);											break;
			case '7': new_member = openFile(&pre_member, &next_member, &t,&fileOpen);break;
			case '8': saveFile(&pre_member);										break;
			case '0': end++;														break;
			default : printf("\n入力が間違っています\n\n"); 							break;
		}
		if(end){
	...
	//count=Count(pre_member);
	free(new_member);	
	free(pre_member);	
	//if( count>=1 || fileOpen==0 ){				
	free(next_member);
...
void sortMenu(member **pre_member){
	...
	char AoD[SIZE],Word[20],choice[SIZE]= {0};	
	int changeWord,ascOrDesc,end;	
			
	while(1){
		end=0;

		memset(choice,'0',(SIZE+1)*sizeof(char));
		...
		switch(choice[0]){
			case 'A': changeWord=1; strcpy(Word,"  名前  ");			break;
			case 'B': changeWord=2;	strcpy(Word,"  住所  ");			break;
			case 'C': changeWord=3;	strcpy(Word,"生年月日");		break;
			case 'D': changeWord=4;	strcpy(Word,"電話番号");		break;
			case 'Z': printf("\nメイン画面に戻ります。\n\n"); end++;	break;
			default : printf("\n入力が間違っています。\n\n"); end++; 	break;
		}
		if(end){
		...
		switch(choice[0]){
			case '1': ascOrDesc=1;	strcpy(AoD,"昇順");			break;
			case '2': ascOrDesc=2;  strcpy(AoD,"降順");			break;
			case '0': printf("\nメイン画面に戻ります。\n\n"); end++;	break;
			default : printf("\n入力が間違っています。\n\n"); end++;	break;
		}
		if(end){
		...
		switch(choice[0]){		
			case '1': doSort(pre_member,changeWord,ascOrDesc);		 break;
			case '2': printf("\nメイン画面に戻ります。\n\n"); 		 break;
			default : printf("\n入力が間違っています。\n\n");   		 break;
		}	
		break;
 ...
 * ソートの基準情報関数
 * @param (*pre_member)		最初会員の情報
 * @param (*changeWord)		ソートする文字列
 * @param (*ascOrDesc)		昇順or降順
 * @return  			ソートの実行に移る
*/
void doSort(member **pre_member, int changeWord, int ascOrDesc){
			...
			for( i=0 ;i<count ;i++){
				...
				while (person-> next !=NULL) {
					switch(changeWord){
						case 1:
								switch(ascOrDesc){
									case 1: doIf=(strcmp(person->name, person->next->name)>0);break;
									case 2: doIf=(strcmp(person->name, person->next->name)<0);break;
									default : break;
								}
								break;
						case 2:
								switch(ascOrDesc){
									case 1: doIf=(strcmp(person->address, person->next->address)>0);break;
									case 2: doIf=(strcmp(person->address, person->next->address)<0);break;
									default : break;
								}
								break;			
						case 3:
								switch(ascOrDesc){
									case 1: doIf=(strcmp(person->birth, person->next->birth)>0);break;
									case 2: doIf=(strcmp(person->birth, person->next->birth)<0);break;
									default : break;
								}
								break;
						case 4:
								switch(ascOrDesc){
									case 1: doIf=(strcmp(person->tel, person->next->tel)>0);break;
									case 2: doIf=(strcmp(person->tel, person->next->tel)<0);break;
									default : break;
								}
								break;
						default:break;		
...
void swap(member* person, member**pre_member){ 
    ...
    if (person->pre != NULL) {
        person->pre->next = temp;
    }
	
    temp->pre = person->pre;
    person->pre = temp;
	
    if (person == (*pre_member)) {
        (*pre_member) = temp;
    }
	
    if (person->next != NULL) {
        person->next->pre = person;
    }
    return;
...
/**
 * 全会員の数を数える関数
 * @param (*person)		会員の情報（最初会員の情報でもらってる）
 * @return  			全会員の数
*/
int Count(member *person){
	int count=0 ; 
